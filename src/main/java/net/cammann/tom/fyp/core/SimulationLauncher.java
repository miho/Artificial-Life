...
public final class SimulationLauncher {
	...
	public static void main(final String[] args) {
		...
		logger.info("Starting evolution module lab");

		final StatsPackage stats = new StatsPackage();

		final BestLifeLauncher lifelauncher = new BestLifeLauncher(
				evolutionModule, factory);

		LoggingFrame.getInstance().setVisible("simCon", true);

		factory.setMapHeight(100);
		factory.setMapWidth(900);
		factory.setNumOfResources(50);
		factory.setLenOfFitFuncRun(50);
		factory.setNumOfObstacles(40);
		evolutionModule.setMaxGenerations(60);
		evolutionModule.setPopulationSize(10);

		logger.info("Generations to run: "
				+ evolutionModule.getNumGenerations());
		// g.setPopulationSize(100);

		lifelauncher.createAndShowGui();
		...
		createAndShowFromFactory(factory, evolutionModule.getFittestLife());
